...
/// @see AsciiDocToText
/// @see AsciiDocToSvg
public final class AsciiDocToHtml implements Function<AsciiDocModel, String> {
            ...
            case Table(List<Row> rows) -> {
                ...
                for (var row : rows) {
                    ...
                    for (var cell : row.cells()) {
                        sb.append("    <td>")
                                .append(cell.blocks().stream().map(AsciiDocToHtml::renderBlock).collect(joining()))
                                .append("</td>\n");
                    }
                    sb.append("  </tr>\n");
                }
    ...
    private static String renderInlines(List<Inline> inlines) {
        ...
        for (var inline : inlines) {
            switch (inline) {
                ...
                case Bold(List<Inline> children) -> sb.append("<b>").append(renderInlines(children)).append("</b>");
                case Italic(List<Inline> children) -> sb.append("<i>").append(renderInlines(children)).append("</i>");
